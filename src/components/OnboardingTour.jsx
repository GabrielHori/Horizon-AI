...
const OnboardingTour = ({ onComplete, language = 'fr', setActiveTab }) => {
  ...
  const getCardStyle = () => {
    ...
    const preferredLeft = Math.min(
      ...
      viewportW - maxWidth - margin
    );

    const belowFits = preferredTop + 260 < viewportH;
    const top = belowFits
      ? preferredTop
      : Math.max(margin, targetRect.top - margin - 240);
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center">
      ...
      <div
        className="relative z-20 glass-panel rounded-[28px] border border-white/10 shadow-2xl overflow-hidden prism-edge"
        style={getCardStyle()}
      >
        <button
          onClick={handleSkip}
